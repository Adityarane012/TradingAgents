...
def _write(tmp_path, name="AAPL-YFin-data.csv", age_seconds=0.0, last_date="2026-07-17"):
    ...
    # mktime, not NOW.timestamp(): a naive Timestamp's .timestamp() treats it
    # as UTC, while _cache_is_fresh reads the mtime back with
    # pd.Timestamp.fromtimestamp(), which renders in local time. Mixing the two
    # offsets the age by the local UTC offset, so in IST (+5:30) a file written
    # 960s ago measured as -18,840s and a stale cache tested as fresh. mktime
    # interprets the same wall-clock as local, matching how it is read back.
    written = time.mktime(NOW.timetuple()) - age_seconds
